...
impl BitcoinCoreSv2TDP {
    pub(crate) async fn handle_coinbase_output_constraints(
        ...
        // Wait for the old monitor_ipc_templates task to finish before replacing
        // current_template_ipc_client.
        //
        // This ordering is important: when monitor_ipc_templates() observes the cancellation token,
        // it calls interrupt_wait_request(), which reads current_template_ipc_client to interrupt
        // the in-flight waitNext request. If we installed the new template IPC client first, the
        // old monitor task could accidentally interrupt the newly bootstrapped client
