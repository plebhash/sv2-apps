...
impl BitcoinCoreSv2JDP {
    ...
    /// state and stale transactions never seed a freshly-cleared mirror. A rejection is classified
    /// from the reason Core gives, with no further IPC: `stale-chain-tip` when Core reports that
    /// the tip moved or that the coinbase height is obsolete, `invalid-job` otherwise. Returns
    /// success with current template parameters or an error if validation fails.
