...
use stratum_apps::{
    ...
    stratum_core::{
        framing_sv2::framing::MessageFrame,
...
) -> Option<(Receiver<InboundFrame>, Sender<OutboundFrame>)> {
        ...
        if let Some(action) = action {
            match action {
                ...
                InterceptAction::ReplaceMessage(intercept_message) => {
                    let intercept_frame = MessageFrame::from_message(
...
pub fn message_from_frame(frame: &mut InboundFrame) -> (MsgType, AnyMessageOwned) {
