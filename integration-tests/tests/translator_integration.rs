...
// This test launches a tProxy in aggregated modeand leverages a MockUpstream to test the correct
// functionalities of grouping standard channels.
#[tokio::test]
async fn aggregated_translator_correctly_deals_with_group_channels() {
    ...
    // aggregated tProxy
    let (_, tproxy_addr) = start_sv2_translator(&[sniffer_addr], true, vec![], vec![]).await;
...
// correct functionalities of grouping standard channels.
#[tokio::test]
async fn non_aggregated_translator_correctly_deals_with_group_channels() {
    ...
    let (_, tproxy_addr) = start_sv2_translator(&[sniffer_addr], false, vec![], vec![]).await;
