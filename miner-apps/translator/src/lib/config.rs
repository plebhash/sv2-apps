...
pub struct TranslatorConfig {
    ...
    ///
    /// `None` and `Some(0)` both select the `channels_sv2` default. This is a retention
    /// window — `cap / job rate` — so the right value depends on how fast the upstream sends
    /// jobs and on this deployment's SV1 miners' submission latency.
    #[serde(default)]
