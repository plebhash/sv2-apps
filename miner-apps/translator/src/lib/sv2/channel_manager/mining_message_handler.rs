        ...
        let success = self
            ...
            .safe_lock(|channel_manager_data| {
                ...
                match channel_manager_data.group_channels.get(&m.group_channel_id) {
                    Some(group_channel_arc) => {
                        ...
                        group_channel.add_channel_id(m.channel_id, full_extranonce_size).map_err(|e| {
                            ...
                            TproxyError::FailedToAddChannelIdToGroupChannel(e)
                        })?;
                    }
                    None => {
                ...
                if channel_manager_data.mode == ChannelMode::Aggregated {
                    ...
                    Ok::<OpenExtendedMiningChannelSuccess<'static>, TproxyError>(new_open_extended_mining_channel_success.into_static())
                } else {
                    ...
                    if m.extranonce_size as usize != downstream_extranonce_len {
                        ...
                        Ok::<OpenExtendedMiningChannelSuccess<'static>, TproxyError>(m.into_static())
                    }
                }
            })
            .map_err(|e| {
                ...
                .safe_lock(|channel_manage_data| {
                        ...
                        channel_manage_data.group_channels.get(&m.channel_id)
                    {
                                ...
                                .contains(&aggregated_channel.get_channel_id())
                            {
                                ...
                                aggregated_channel
                                    ...
                                    .map_err(|e| {
                                        ...
                                        TproxyError::FailedToProcessNewExtendedMiningJob
                                    })?;
                                ...
                                if !m_static.is_future() {
                                    ...
                                    new_extended_mining_job_message.channel_id =
                                        AGGREGATED_CHANNEL_ID; // this is done so that every aggregated downstream
                                ...
                                return Ok::<Vec<NewExtendedMiningJob<'static>>, TproxyError>(
                                    new_extended_mining_job_messages,
                                );
                            }
                        }
                        ...
                        for channel_id in group_channel.get_channel_ids() {
                            let channel = channel_manage_data
                                .extended_channels
                                .get(channel_id)
                                .ok_or(TproxyError::ChannelNotFound)?;
                            let mut channel = channel.write().map_err(|e| {
                                error!("Failed to write channel: {:?}", e);
                                TproxyError::PoisonLock
                            })?;
                    ...
                    // working in aggregated mode
                        ...
                        if aggregated_channel.get_channel_id() == m_static.channel_id {
                            ...
                            return Err(TproxyError::ChannelNotFound);
                        }
                    ...
                    // mode, we need to process the message for a specific channel
                        ...
                        let channel = channel_manage_data
                            ...
                            .ok_or(TproxyError::ChannelNotFound)?; // we got a nonsense channel id, we should trigger fallback
                    ...
                    Ok::<Vec<NewExtendedMiningJob<'static>>, TproxyError>(
                        new_extended_mining_job_messages,
                    )
                })??;
                        ...
                        channel_manager_data.group_channels.get(&m.channel_id)
                    {
                        ...
                        group_channel
                            ...
                            .map_err(|e| {
                                ...
                                TproxyError::FailedToProcessSetNewPrevHash
                            })?;
        ...
        let set_target_messages_sv1_server =
            self.channel_manager_data
                .safe_lock(|channel_manager_data| {
                        ...
                        channel_manager_data.group_channels.get(&m.channel_id)
                    {
                            ...
                            channel_manager_data.upstream_extended_channel.as_ref()
                        {
                                ...
                                .contains(&aggregated_channel.get_channel_id())
                            {
                                ...
                                // exit the closure early, there's nothing else to do
                                return Ok::<Vec<SetTarget<'static>>, TproxyError>(
                                    set_target_messages,
                                );
                            }
                        }
                    ...
                    Ok::<Vec<SetTarget<'static>>, TproxyError>(set_target_messages)
                })??;
