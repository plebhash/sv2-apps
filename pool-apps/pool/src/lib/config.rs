...
pub struct PoolConfig {
    ...
    ///
    /// `None` (the default) and `Some(0)` both select the `channels_sv2` default. The cap is
    /// really a retention window — `cap / job rate` — and the job rate belongs to this
    /// deployment, not to the library, which is why it is settable here.
    ///
    /// Do not lower it on a pool serving job-declaration clients: every accepted
    /// `SetCustomMiningJob` retires the active job, so the job rate there is the client's
    /// rather than this pool's, and nothing here bounds it.
    #[serde(default)]
